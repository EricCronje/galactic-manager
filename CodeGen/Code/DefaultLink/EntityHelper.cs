    ...
    public class ~Entity~Helper : IHelper<~Entity~>, IDisposable
	{
        ...
        public bool Add(int? simChoice = null, string[]? simInput = null)
        {
			...
			if (!string.IsNullOrWhiteSpace(Input))
			{
                ...
                {
					var LhHelper = AL.~LhLink~Helper;
					var RhHelper = AL.~RhLink~Helper;
					var selectionHeading = "Select the item from the ~LhLink~";
            ...
            try
            {
                ...
                if (Entity != null)
                {
                    ...
                    if (!string.IsNullOrWhiteSpace(Input))
                    {
                        ...
                        if (NotFound == null)
                        {
	                        EntitiesOnThePage = [new(Input, 0, Entity?.LhGuid, Entity?.RhGuid)];
        ...
        public bool Search(int? simChoice = null, string[]? simInput = null)
        {
            ...
            else
            {
                ...
                else
                {
                    ...
                    EntitiesOnThePage = [new ~Entity~("None", 1, null, null)];
                    Page = new Patina.Patina(1, 1);
                    return false;
                }
            }
        ...
        public bool Save(int? simChoice = null, string[]? simInput = null)
		{
            ...
            if (Repository?.Count() > 0)
            {
                ...
                try
                {
					sb.AppendLine($"~Entity~Name|lhLink|rhLink|lhLinkName|rhLinkName|Guid|lhLinkGuid|rhLinkGuid");
                    var OrderedList = Repository.OrderByName();
                    if (OrderedList != null)
                    {
                        foreach (~Entity~ item in OrderedList)
                        {
							sb.AppendLine($"{ item.Name}|~LhLink~|~RhLink~|{AL.~LhLink~Helper?.GetName(item.LhGuid)}|{AL.~RhLink~Helper?.GetName(item.RhGuid)}|{ item.Guid_}|{item.LhGuid}|{item.RhGuid}");
                        }
                        if (File.Exists(Path))
                            File.Delete(Path);
	                    ...
	                    if (!SkipFirstLine)
	                    {
							...
							if (LineItem.Length > 0 && LineItem.Length > 1)
	                        {
								...
								if (Entity == null)
								{
									...
									Repository?.Add(new ~Entity~(Name, 0, lhLinkGuid, rhLinkGuid, GUID));
