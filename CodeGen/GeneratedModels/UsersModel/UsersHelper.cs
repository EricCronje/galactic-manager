...
using ArchCorpUtilities.Models.UsersModel.Users;
using ArchCorpUtilities.Models.Helper;
using System.Text;

namespace ArchCorpUtilities.GeneratedModels.UsersModel
{
    public class UsersHelper : IHelper<Users>, IDisposable
	{
        public string? SessionID { get; set; }
        public List<Users>? Items { get; set; }
        public List<Users>? EntitiesOnThePage { get; set; }
        public Patina.Patina Page { get; set; }

       public UsersHelper(string? sessionID)
		{
            SessionID = sessionID;
            Items = MockData();
            Page = new(Convert.ToUInt32(5), Convert.ToUInt32(Items?.Count));
		}
        private List<Users>? MockData()
			{
            if (Items == null || Items.Count == 0)
                Items = [];

            Items.Add(new Users("Alpha-Users", 1));
            Items.Add(new Users("Beta-Users", 2));
            Items.Add(new Users("Charlie-Users", 3));
            Items.Add(new Users("Delta-Users", 3));
            Items.Add(new Users("Echo-Users", 3));
            Items.Add(new Users("Foxtrot-Users", 3));
            Items.Add(new Users("Golf-Users", 3));
            Items.Add(new Users("Hotel-Users", 3));
            return Items;
			}
        ...
        public bool View(U.Navigation navigate = U.Navigation.FirstPage)
           ...
           EntitiesOnThePage = U.ViewWithPagination("Users", Page, orderedEntities, navigate);
            return true;
			}
        ...
        public bool Edit(int? simChoice, string[]? simInput)
        {
	        ...
	        try
	        {
	            Users? Entity = ViewAndSelectItem(simInput?[0], "Select an item to edit");
        ...
        public bool Search(int? simChoice = null, string[]? simInput = null)
            ...
            else 
            {
                ...
                List<Users>? Entities = Items?.Where(c => c.Name != null && c.Name.ToUpper().Contains(Input.ToUpper(), StringComparison.CurrentCultureIgnoreCase)).ToList<Users>();
                ...
                else 
                {
                    ...
                    EntitiesOnThePage = [new Users("None", 1)];
                    Page = new Patina.Patina(1, 1);
                    return false;
                }
            }
        ...
        public bool Refresh(List<Users> modelList, U.Navigation navigate = U.Navigation.FirstPage)
        {
            if (SessionID != null)
                L.Log(System.Reflection.MethodBase.GetCurrentMethod()?.Name, SessionID);
        ...
        public bool Remove(int? simChoice = null, string[]? simInput = null)
            ...
            //List the items
            var CurrentUsers = ViewAndSelectItem(simInput?[0], "Select the item to remove");

            if (Items != null && CurrentUsers != null)
                if (Items.Remove(CurrentUsers))
                {
        ...
        public bool Save(int? simChoice = null, string[]? simInput = null)
            ...
            if (Items != null && Items.Count > 0)
            {
                ...
                try
                {
                    sb.AppendLine($"UsersName|UsersGuid");
                    foreach (var item in Items.OrderBy(c => c.Name))
                    {
                        sb.AppendLine($"{ item.Name}|{ item.UsersGuid}");
                    }
	                ...
	                foreach (string line in FileInput.Split("\r\n"))
	                {
	                    if (!SkipFirstLine)
	                    {
							...
							if (LineItem.Length > 0 && LineItem.Length > 1)
	                        {
	                            ...
	                            var Entity = Items?.FirstOrDefault(c => c.UsersGuid == GUID);
	                            var EntityItem = Items?.FirstOrDefault(c => c.Name == Name);
								var OldGUID = CH.IsSimulate ? "<GUID>" : EntityItem?.UsersGuid;
								...
								if (Entity == null)
								{
									...
									Items?.Add(new Users(Name, 0, GUID));
        ...
        private Users? ViewAndSelectItem(string? simInput, string heading)
        {
        ...
        {
            return Items?.FirstOrDefault(p => p.UsersGuid != null && p.UsersGuid.Equals(guid))?.Name;
        }

        internal string? GetGuid(string? name)
        {
            return Items?.FirstOrDefault(p => p.Name != null && p.Name.Equals(name))?.UsersGuid;
        }
    }
}
