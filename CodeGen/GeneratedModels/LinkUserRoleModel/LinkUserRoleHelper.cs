...
{
    public class LinkUserRoleHelper : IHelper<LinkUserRole>, IDisposable
	{
        public string? SessionID { get; set; }
        public List<LinkUserRole>? Items { get; set; }
        public List<LinkUserRole>? EntitiesOnThePage { get; set; }
        public Patina.Patina Page { get; set; }
        public LinkUserRoleMockRepository<LinkUserRole> Repository { get; set; }
       public LinkUserRoleHelper(string? sessionID)
		{
            SessionID = sessionID;
            Items = [];
            Page = new(Convert.ToUInt32(5), Convert.ToUInt32(Items?.Count));
            Repository = new("-LinkUserRole");
		}
        public bool View(U.Navigation navigate = U.Navigation.FirstPage)
           ...
           EntitiesOnThePage = U.ViewWithPagination("LinkUserRole", Page, orderedEntities, navigate);
            return true;
			}
        ...
        public bool Edit(int? simChoice, string[]? simInput)
        {
	        ...
	        try
	        {
	            LinkUserRole? Entity = ViewAndSelectItem(simInput?[0], "Select an item to edit");
	            ...
	            if (Entity != null)
	            {
	                ...
	                if (!string.IsNullOrWhiteSpace(Input))
	                {
	                    ...
	                    if (NotFound == null)
	                    {
	                        EntitiesOnThePage = [new(Input, 0, Entity?.UsersGuid, Entity?.RolesGuid)];
        ...
        public bool Search(int? simChoice = null, string[]? simInput = null)
            ...
            else 
            {
                ...
                List<LinkUserRole>? Entities = Items?.Where(c => c.Name != null && c.Name.ToUpper().Contains(Input.ToUpper(), StringComparison.CurrentCultureIgnoreCase)).ToList<LinkUserRole>();
                ...
                else 
                {
                    ...
                    EntitiesOnThePage = [new LinkUserRole("None", 1, null, null)];
                    Page = new Patina.Patina(1, 1);
                    return false;
                }
            }
        ...
        public bool Refresh(List<LinkUserRole> modelList, U.Navigation navigate = U.Navigation.FirstPage)
        {
            if (SessionID != null) { L.Log(System.Reflection.MethodBase.GetCurrentMethod()?.Name, SessionID);}
            ReIndexDisplayId();
            ResetPageMaxCount();
        ...
        public bool Remove(int? simChoice = null, string[]? simInput = null)
            ...
            //List the items
            var CurrentLinkUserRole = ViewAndSelectItem(simInput?[0], "Select the item to remove");

            if (Items != null && CurrentLinkUserRole != null)
                if (Items.Remove(CurrentLinkUserRole))
                {
        ...
        public bool Save(int? simChoice = null, string[]? simInput = null)
            ...
            if (Items != null && Items.Count > 0)
            {
                ...
                try
                {
                    sb.AppendLine($"LinkUserRoleName|LHLink|RHLink|LHLinkName|RHLinkName|Guid|LHLinkGuid|RHLinkGuid");
                    foreach (var item in Items.OrderBy(c => c.Name))
                    {
                        sb.AppendLine($"{ item.Name}|Users|Roles|{AL.UsersHelper?.GetName(item.UsersGuid)}|{AL.RolesHelper?.GetName(item.RolesGuid)}|{ item.LinkUserRoleGuid}|{item.UsersGuid}|{item.RolesGuid}");
                    }
	                ...
	                foreach (string line in FileInput.Split("\r\n"))
	                {
	                    if (!SkipFirstLine)
	                    {
							...
							if (LineItem.Length > 0 && LineItem.Length > 1)
	                        {
	                            ...
	                            var Entity = Items?.FirstOrDefault(c => c.LinkUserRoleGuid == GUID);
	                            var EntityItem = Items?.FirstOrDefault(c => c.Name == Name);
								var OldGUID = CH.IsSimulate ? "<GUID>" : EntityItem?.LinkUserRoleGuid;
								...
								if (Entity == null)
								{
									...
									Items?.Add(new LinkUserRole(Name, 0, LHLinkGuid, RHLinkGuid, GUID));
        ...
        public void ReIndexDisplayId()
        {
            ...
            if (OrderedModels != null)
                ...
                {
                    LinkUserRole? item = OrderedModels[i];
                    item.DisplayId = i + 1;
        ...
        private LinkUserRole? ViewAndSelectItem(string? simInput, string heading)
        {
        ...
        {
            return Items?.FirstOrDefault(p => p.LinkUserRoleGuid != null && p.LinkUserRoleGuid.Equals(guid))?.Name;
        }
        internal string? GetGuid(string? name)
        {
            return Items?.FirstOrDefault(p => p.Name != null && p.Name.Equals(name))?.LinkUserRoleGuid;
        }
	}
}
