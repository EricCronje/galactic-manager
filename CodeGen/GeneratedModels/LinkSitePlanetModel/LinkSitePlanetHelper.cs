...
{
    public class LinkSitePlanetHelper : IHelper<LinkSitePlanet>, IDisposable
	{
        public string? SessionID { get; set; }
        public List<LinkSitePlanet>? Items { get; set; }
        public List<LinkSitePlanet>? EntitiesOnThePage { get; set; }
        public Patina.Patina Page { get; set; }
        public LinkSitePlanetMockRepository<LinkSitePlanet> Repository { get; set; }
       public LinkSitePlanetHelper(string? sessionID)
		{
            SessionID = sessionID;
            Items = [];
            Page = new(Convert.ToUInt32(5), Convert.ToUInt32(Items?.Count));
            Repository = new("-LinkSitePlanet");
		}
        public bool View(U.Navigation navigate = U.Navigation.FirstPage)
           ...
           EntitiesOnThePage = U.ViewWithPagination("LinkSitePlanet", Page, orderedEntities, navigate);
            return true;
			}
        ...
        public bool Edit(int? simChoice, string[]? simInput)
        {
	        ...
	        try
	        {
	            LinkSitePlanet? Entity = ViewAndSelectItem(simInput?[0], "Select an item to edit");
        ...
        public bool Search(int? simChoice = null, string[]? simInput = null)
            ...
            else 
            {
                ...
                List<LinkSitePlanet>? Entities = Items?.Where(c => c.Name != null && c.Name.ToUpper().Contains(Input.ToUpper(), StringComparison.CurrentCultureIgnoreCase)).ToList<LinkSitePlanet>();
                ...
                else 
                {
                    ...
                    EntitiesOnThePage = [new LinkSitePlanet("None", 1, null, null)];
                    Page = new Patina.Patina(1, 1);
                    return false;
                }
            }
        ...
        public bool Refresh(List<LinkSitePlanet> modelList, U.Navigation navigate = U.Navigation.FirstPage)
        {
            if (SessionID != null) { L.Log(System.Reflection.MethodBase.GetCurrentMethod()?.Name, SessionID);}
            ReIndexDisplayId();
            ResetPageMaxCount();
        ...
        public bool Remove(int? simChoice = null, string[]? simInput = null)
            ...
            //List the items
            var CurrentLinkSitePlanet = ViewAndSelectItem(simInput?[0], "Select the item to remove");

            if (Items != null && CurrentLinkSitePlanet != null)
                if (Items.Remove(CurrentLinkSitePlanet))
                {
        ...
        public bool Save(int? simChoice = null, string[]? simInput = null)
            ...
            if (Items != null && Items.Count > 0)
            {
                ...
                try
                {
                    sb.AppendLine($"LinkSitePlanetName|LHLink|RHLink|LHLinkName|RHLinkName|Guid|LHLinkGuid|RHLinkGuid");
                    foreach (var item in Items.OrderBy(c => c.Name))
                    {
                        sb.AppendLine($"{ item.Name}|Sites|Planets|{AL.SitesHelper?.GetName(item.SitesGuid)}|{AL.PlanetsHelper?.GetName(item.PlanetsGuid)}|{ item.LinkSitePlanetGuid}|{item.SitesGuid}|{item.PlanetsGuid}");
                    }
	                ...
	                foreach (string line in FileInput.Split("\r\n"))
	                {
	                    if (!SkipFirstLine)
	                    {
							...
							if (LineItem.Length > 0 && LineItem.Length > 1)
	                        {
	                            ...
	                            var Entity = Items?.FirstOrDefault(c => c.LinkSitePlanetGuid == GUID);
	                            var EntityItem = Items?.FirstOrDefault(c => c.Name == Name);
								var OldGUID = CH.IsSimulate ? "<GUID>" : EntityItem?.LinkSitePlanetGuid;
								...
								if (Entity == null)
								{
									...
									Items?.Add(new LinkSitePlanet(Name, 0, LHLinkGuid, RHLinkGuid, GUID));
        ...
        public void ReIndexDisplayId()
        {
            ...
            if (OrderedModels != null)
                ...
                {
                    LinkSitePlanet? item = OrderedModels[i];
                    item.DisplayId = i + 1;
        ...
        private LinkSitePlanet? ViewAndSelectItem(string? simInput, string heading)
        {
        ...
        {
            return Items?.FirstOrDefault(p => p.LinkSitePlanetGuid != null && p.LinkSitePlanetGuid.Equals(guid))?.Name;
        }
        internal string? GetGuid(string? name)
        {
            return Items?.FirstOrDefault(p => p.Name != null && p.Name.Equals(name))?.LinkSitePlanetGuid;
        }
	}
}
