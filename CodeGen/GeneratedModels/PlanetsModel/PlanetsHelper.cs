...
using ArchCorpUtilities.Models.PlanetsModel.Planets;
using ArchCorpUtilities.Models.Helper;
using System.Text;

namespace ArchCorpUtilities.GeneratedModels.PlanetsModel
{
    public class PlanetsHelper : IHelper<Planets>, IDisposable
	{
        public string? SessionID { get; set; }
        public List<Planets>? Items { get; set; }
        public List<Planets>? EntitiesOnThePage { get; set; }
        public Patina.Patina Page { get; set; }

       public PlanetsHelper(string? sessionID)
		{
            SessionID = sessionID;
            Items = MockData();
            Page = new(Convert.ToUInt32(5), Convert.ToUInt32(Items?.Count));
		}
        private List<Planets>? MockData()
			{
            if (Items == null || Items.Count == 0)
                Items = [];

            Items.Add(new Planets("Alpha-Planets", 1));
            Items.Add(new Planets("Beta-Planets", 2));
            Items.Add(new Planets("Charlie-Planets", 3));
            Items.Add(new Planets("Delta-Planets", 3));
            Items.Add(new Planets("Echo-Planets", 3));
            Items.Add(new Planets("Foxtrot-Planets", 3));
            Items.Add(new Planets("Golf-Planets", 3));
            Items.Add(new Planets("Hotel-Planets", 3));
            return Items;
			}
        ...
        public bool View(U.Navigation navigate = U.Navigation.FirstPage)
           ...
           EntitiesOnThePage = U.ViewWithPagination("Planets", Page, orderedEntities, navigate);
            return true;
			}
        ...
        public bool Edit(int? simChoice, string[]? simInput)
        {
	        ...
	        try
	        {
	            Planets? Entity = ViewAndSelectItem(simInput?[0], "Select an item to edit");
        ...
        public bool Search(int? simChoice = null, string[]? simInput = null)
            ...
            else 
            {
                ...
                List<Planets>? Entities = Items?.Where(c => c.Name != null && c.Name.ToUpper().Contains(Input.ToUpper(), StringComparison.CurrentCultureIgnoreCase)).ToList<Planets>();
                ...
                else 
                {
                    ...
                    EntitiesOnThePage = [new Planets("None", 1)];
                    Page = new Patina.Patina(1, 1);
                    return false;
                }
            }
        ...
        public bool Refresh(List<Planets> modelList, U.Navigation navigate = U.Navigation.FirstPage)
        {
            if (SessionID != null)
                L.Log(System.Reflection.MethodBase.GetCurrentMethod()?.Name, SessionID);
        ...
        public bool Remove(int? simChoice = null, string[]? simInput = null)
            ...
            //List the items
            var CurrentPlanets = ViewAndSelectItem(simInput?[0], "Select the item to remove");

            if (Items != null && CurrentPlanets != null)
                if (Items.Remove(CurrentPlanets))
                {
        ...
        public bool Save(int? simChoice = null, string[]? simInput = null)
            ...
            if (Items != null && Items.Count > 0)
            {
                ...
                try
                {
                    sb.AppendLine($"PlanetsName|PlanetsGuid");
                    foreach (var item in Items.OrderBy(c => c.Name))
                    {
                        sb.AppendLine($"{ item.Name}|{ item.PlanetsGuid}");
                    }
	                ...
	                foreach (string line in FileInput.Split("\r\n"))
	                {
	                    if (!SkipFirstLine)
	                    {
							...
							if (LineItem.Length > 0 && LineItem.Length > 1)
	                        {
	                            ...
	                            var Entity = Items?.FirstOrDefault(c => c.PlanetsGuid == GUID);
	                            var EntityItem = Items?.FirstOrDefault(c => c.Name == Name);
								var OldGUID = CH.IsSimulate ? "<GUID>" : EntityItem?.PlanetsGuid;
								...
								if (Entity == null)
								{
									...
									Items?.Add(new Planets(Name, 0, GUID));
        ...
        private Planets? ViewAndSelectItem(string? simInput, string heading)
        {
        ...
        {
            return Items?.FirstOrDefault(p => p.PlanetsGuid != null && p.PlanetsGuid.Equals(guid))?.Name;
        }

        internal string? GetGuid(string? name)
        {
            return Items?.FirstOrDefault(p => p.Name != null && p.Name.Equals(name))?.PlanetsGuid;
        }
    }
}
