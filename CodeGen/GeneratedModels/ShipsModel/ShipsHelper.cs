...
using ArchCorpUtilities.Models.ShipsModel.Ships;
using ArchCorpUtilities.Models.Helper;
using System.Text;

namespace ArchCorpUtilities.GeneratedModels.ShipsModel
{
    public class ShipsHelper : IHelper<Ships>, IDisposable
	{
        public string? SessionID { get; set; }
        public List<Ships>? Items { get; set; }
        public List<Ships>? EntitiesOnThePage { get; set; }
        public Patina.Patina Page { get; set; }

       public ShipsHelper(string? sessionID)
		{
            SessionID = sessionID;
            Items = MockData();
            Page = new(Convert.ToUInt32(5), Convert.ToUInt32(Items?.Count));
		}
        private List<Ships>? MockData()
			{
            if (Items == null || Items.Count == 0)
                Items = [];

            Items.Add(new Ships("Alpha-Ships", 1));
            Items.Add(new Ships("Beta-Ships", 2));
            Items.Add(new Ships("Charlie-Ships", 3));
            Items.Add(new Ships("Delta-Ships", 3));
            Items.Add(new Ships("Echo-Ships", 3));
            Items.Add(new Ships("Foxtrot-Ships", 3));
            Items.Add(new Ships("Golf-Ships", 3));
            Items.Add(new Ships("Hotel-Ships", 3));
            return Items;
			}
        ...
        public bool View(U.Navigation navigate = U.Navigation.FirstPage)
           ...
           EntitiesOnThePage = U.ViewWithPagination("Ships", Page, orderedEntities, navigate);
            return true;
			}
        ...
        public bool Edit(int? simChoice, string[]? simInput)
        {
	        ...
	        try
	        {
	            Ships? Entity = ViewAndSelectItem(simInput?[0], "Select an item to edit");
        ...
        public bool Search(int? simChoice = null, string[]? simInput = null)
            ...
            else 
            {
                ...
                List<Ships>? Entities = Items?.Where(c => c.Name != null && c.Name.ToUpper().Contains(Input.ToUpper(), StringComparison.CurrentCultureIgnoreCase)).ToList<Ships>();
                ...
                else 
                {
                    ...
                    EntitiesOnThePage = [new Ships("None", 1)];
                    Page = new Patina.Patina(1, 1);
                    return false;
                }
            }
        ...
        public bool Refresh(List<Ships> modelList, U.Navigation navigate = U.Navigation.FirstPage)
        {
            if (SessionID != null)
                L.Log(System.Reflection.MethodBase.GetCurrentMethod()?.Name, SessionID);
        ...
        public bool Remove(int? simChoice = null, string[]? simInput = null)
            ...
            //List the items
            var CurrentShips = ViewAndSelectItem(simInput?[0], "Select the item to remove");

            if (Items != null && CurrentShips != null)
                if (Items.Remove(CurrentShips))
                {
        ...
        public bool Save(int? simChoice = null, string[]? simInput = null)
            ...
            if (Items != null && Items.Count > 0)
            {
                ...
                try
                {
                    sb.AppendLine($"ShipsName|ShipsGuid");
                    foreach (var item in Items.OrderBy(c => c.Name))
                    {
                        sb.AppendLine($"{ item.Name}|{ item.ShipsGuid}");
                    }
	                ...
	                foreach (string line in FileInput.Split("\r\n"))
	                {
	                    if (!SkipFirstLine)
	                    {
							...
							if (LineItem.Length > 0 && LineItem.Length > 1)
	                        {
	                            ...
	                            var Entity = Items?.FirstOrDefault(c => c.ShipsGuid == GUID);
	                            var EntityItem = Items?.FirstOrDefault(c => c.Name == Name);
								var OldGUID = CH.IsSimulate ? "<GUID>" : EntityItem?.ShipsGuid;
								...
								if (Entity == null)
								{
									...
									Items?.Add(new Ships(Name, 0, GUID));
        ...
        private Ships? ViewAndSelectItem(string? simInput, string heading)
        {
        ...
        {
            return Items?.FirstOrDefault(p => p.ShipsGuid != null && p.ShipsGuid.Equals(guid))?.Name;
        }

        internal string? GetGuid(string? name)
        {
            return Items?.FirstOrDefault(p => p.Name != null && p.Name.Equals(name))?.ShipsGuid;
        }
    }
}
