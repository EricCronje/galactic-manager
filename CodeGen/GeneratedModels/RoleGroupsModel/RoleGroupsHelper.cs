...
using ArchCorpUtilities.Models.RoleGroupsModel.RoleGroups;
using ArchCorpUtilities.Models.Helper;
using System.Text;

namespace ArchCorpUtilities.GeneratedModels.RoleGroupsModel
{
    public class RoleGroupsHelper : IHelper<RoleGroups>, IDisposable
	{
        public string? SessionID { get; set; }
        public List<RoleGroups>? Items { get; set; }
        public List<RoleGroups>? EntitiesOnThePage { get; set; }
        public Patina.Patina Page { get; set; }

       public RoleGroupsHelper(string? sessionID)
		{
            SessionID = sessionID;
            Items = MockData();
            Page = new(Convert.ToUInt32(5), Convert.ToUInt32(Items?.Count));
		}
        private List<RoleGroups>? MockData()
			{
            if (Items == null || Items.Count == 0)
                Items = [];

            Items.Add(new RoleGroups("Alpha-RoleGroups", 1));
            Items.Add(new RoleGroups("Beta-RoleGroups", 2));
            Items.Add(new RoleGroups("Charlie-RoleGroups", 3));
            Items.Add(new RoleGroups("Delta-RoleGroups", 3));
            Items.Add(new RoleGroups("Echo-RoleGroups", 3));
            Items.Add(new RoleGroups("Foxtrot-RoleGroups", 3));
            Items.Add(new RoleGroups("Golf-RoleGroups", 3));
            Items.Add(new RoleGroups("Hotel-RoleGroups", 3));
            return Items;
			}
        ...
        public bool View(U.Navigation navigate = U.Navigation.FirstPage)
           ...
           EntitiesOnThePage = U.ViewWithPagination("RoleGroups", Page, orderedEntities, navigate);
            return true;
			}
        ...
        public bool Edit(int? simChoice, string[]? simInput)
        {
	        ...
	        try
	        {
	            RoleGroups? Entity = ViewAndSelectItem(simInput?[0], "Select an item to edit");
        ...
        public bool Search(int? simChoice = null, string[]? simInput = null)
            ...
            else 
            {
                ...
                List<RoleGroups>? Entities = Items?.Where(c => c.Name != null && c.Name.ToUpper().Contains(Input.ToUpper(), StringComparison.CurrentCultureIgnoreCase)).ToList<RoleGroups>();
                ...
                else 
                {
                    ...
                    EntitiesOnThePage = [new RoleGroups("None", 1)];
                    Page = new Patina.Patina(1, 1);
                    return false;
                }
            }
        ...
        public bool Refresh(List<RoleGroups> modelList, U.Navigation navigate = U.Navigation.FirstPage)
        {
            if (SessionID != null)
                L.Log(System.Reflection.MethodBase.GetCurrentMethod()?.Name, SessionID);
        ...
        public bool Remove(int? simChoice = null, string[]? simInput = null)
            ...
            //List the items
            var CurrentRoleGroups = ViewAndSelectItem(simInput?[0], "Select the item to remove");

            if (Items != null && CurrentRoleGroups != null)
                if (Items.Remove(CurrentRoleGroups))
                {
        ...
        public bool Save(int? simChoice = null, string[]? simInput = null)
            ...
            if (Items != null && Items.Count > 0)
            {
                ...
                try
                {
                    sb.AppendLine($"RoleGroupsName|RoleGroupsGuid");
                    foreach (var item in Items.OrderBy(c => c.Name))
                    {
                        sb.AppendLine($"{ item.Name}|{ item.RoleGroupsGuid}");
                    }
	                ...
	                foreach (string line in FileInput.Split("\r\n"))
	                {
	                    if (!SkipFirstLine)
	                    {
							...
							if (LineItem.Length > 0 && LineItem.Length > 1)
	                        {
	                            ...
	                            var Entity = Items?.FirstOrDefault(c => c.RoleGroupsGuid == GUID);
	                            var EntityItem = Items?.FirstOrDefault(c => c.Name == Name);
								var OldGUID = CH.IsSimulate ? "<GUID>" : EntityItem?.RoleGroupsGuid;
								...
								if (Entity == null)
								{
									...
									Items?.Add(new RoleGroups(Name, 0, GUID));
        ...
        private RoleGroups? ViewAndSelectItem(string? simInput, string heading)
        {
        ...
        {
            return Items?.FirstOrDefault(p => p.RoleGroupsGuid != null && p.RoleGroupsGuid.Equals(guid))?.Name;
        }

        internal string? GetGuid(string? name)
        {
            return Items?.FirstOrDefault(p => p.Name != null && p.Name.Equals(name))?.RoleGroupsGuid;
        }
    }
}
