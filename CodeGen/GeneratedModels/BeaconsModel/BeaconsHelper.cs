       ...
       public BeaconsHelper(string? sessionID)
        ...
        public bool Search(int? simChoice = null, string[]? simInput = null)
            ...
            else 
            {
                ...
                List<Beacons>? Entities = Items?.Where(c => c.Name != null && c.Name.ToUpper().Contains(Input.ToUpper(), StringComparison.CurrentCultureIgnoreCase)).ToList<Beacons>();
                ...
                else 
                {
                    ...
                    EntitiesOnThePage = [new Beacons("None", 1)];
                    Page = new Patina.Patina(1, 1);
                    return false;
                }
            }
        ...
        public bool Remove(int? simChoice = null, string[]? simInput = null)
            ...
            if (Items != null && CurrentBeacons != null)
                if (Items.Remove(CurrentBeacons))
                {
        ...
        public bool Save(int? simChoice = null, string[]? simInput = null)
            ...
            if (Items != null && Items.Count > 0)
            {
                ...
                try
                {
                    sb.AppendLine($"BeaconsName|BeaconsGuid");
                    foreach (var item in Items.OrderBy(c => c.Name))
                    {
                        sb.AppendLine($"{ item.Name}|{ item.BeaconsGuid}");
                    }
