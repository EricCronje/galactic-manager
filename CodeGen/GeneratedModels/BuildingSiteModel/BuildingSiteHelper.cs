...
{
    public class BuildingSiteHelper : IHelper<BuildingSite>, IDisposable
	{
        public string? SessionID { get; set; }
        public List<BuildingSite>? Items { get; set; }
        public List<BuildingSite>? EntitiesOnThePage { get; set; }
        public Patina.Patina Page { get; set; }
        public BuildingSiteMockRepository<BuildingSite> Repository { get; set; }
       public BuildingSiteHelper(string? sessionID)
		{
            SessionID = sessionID;
            Items = [];
            Page = new(Convert.ToUInt32(5), Convert.ToUInt32(Items?.Count));
            Repository = new("-BuildingSite");
		}
        public bool View(U.Navigation navigate = U.Navigation.FirstPage)
           ...
           EntitiesOnThePage = U.ViewWithPagination("BuildingSite", Page, orderedEntities, navigate);
            return true;
			}
        ...
        public bool Edit(int? simChoice, string[]? simInput)
        {
	        ...
	        try
	        {
	            BuildingSite? Entity = ViewAndSelectItem(simInput?[0], "Select an item to edit");
        ...
        public bool Search(int? simChoice = null, string[]? simInput = null)
            ...
            else 
            {
                ...
                List<BuildingSite>? Entities = Items?.Where(c => c.Name != null && c.Name.ToUpper().Contains(Input.ToUpper(), StringComparison.CurrentCultureIgnoreCase)).ToList<BuildingSite>();
                ...
                else 
                {
                    ...
                    EntitiesOnThePage = [new BuildingSite("None", 1, null, null)];
                    Page = new Patina.Patina(1, 1);
                    return false;
                }
            }
        ...
        public bool Refresh(List<BuildingSite> modelList, U.Navigation navigate = U.Navigation.FirstPage)
        {
            if (SessionID != null) { L.Log(System.Reflection.MethodBase.GetCurrentMethod()?.Name, SessionID);}
            ReIndexDisplayId();
            ResetPageMaxCount();
        ...
        public bool Remove(int? simChoice = null, string[]? simInput = null)
            ...
            //List the items
            var CurrentBuildingSite = ViewAndSelectItem(simInput?[0], "Select the item to remove");

            if (Items != null && CurrentBuildingSite != null)
                if (Items.Remove(CurrentBuildingSite))
                {
        ...
        public bool Save(int? simChoice = null, string[]? simInput = null)
            ...
            if (Items != null && Items.Count > 0)
            {
                ...
                try
                {
                    sb.AppendLine($"BuildingSiteName|LHLink|RHLink|LHLinkName|RHLinkName|Guid|LHLinkGuid|RHLinkGuid");
                    foreach (var item in Items.OrderBy(c => c.Name))
                    {
                        sb.AppendLine($"{ item.Name}|Buildings|Sites|{AL.BuildingsHelper?.GetName(item.BuildingsGuid)}|{AL.SitesHelper?.GetName(item.SitesGuid)}|{ item.BuildingSiteGuid}|{item.BuildingsGuid}|{item.SitesGuid}");
                    }
	                ...
	                foreach (string line in FileInput.Split("\r\n"))
	                {
	                    if (!SkipFirstLine)
	                    {
							...
							if (LineItem.Length > 0 && LineItem.Length > 1)
	                        {
	                            ...
	                            var Entity = Items?.FirstOrDefault(c => c.BuildingSiteGuid == GUID);
	                            var EntityItem = Items?.FirstOrDefault(c => c.Name == Name);
								var OldGUID = CH.IsSimulate ? "<GUID>" : EntityItem?.BuildingSiteGuid;
								...
								if (Entity == null)
								{
									...
									Items?.Add(new BuildingSite(Name, 0, LHLinkGuid, RHLinkGuid, GUID));
        ...
        public void ReIndexDisplayId()
        {
            ...
            if (OrderedModels != null)
                ...
                {
                    BuildingSite? item = OrderedModels[i];
                    item.DisplayId = i + 1;
        ...
        private BuildingSite? ViewAndSelectItem(string? simInput, string heading)
        {
        ...
        {
            return Items?.FirstOrDefault(p => p.BuildingSiteGuid != null && p.BuildingSiteGuid.Equals(guid))?.Name;
        }
        internal string? GetGuid(string? name)
        {
            return Items?.FirstOrDefault(p => p.Name != null && p.Name.Equals(name))?.BuildingSiteGuid;
        }
	}
}
