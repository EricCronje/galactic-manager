...
using ArchCorpUtilities.Models.PersonnelModel.Personnel;
using ArchCorpUtilities.Models.Helper;
using System.Text;

namespace ArchCorpUtilities.GeneratedModels.PersonnelModel
{
    public class PersonnelHelper : IHelper<Personnel>, IDisposable
	{
        public string? SessionID { get; set; }
        public List<Personnel>? Items { get; set; }
        public List<Personnel>? EntitiesOnThePage { get; set; }
        public Patina.Patina Page { get; set; }

       public PersonnelHelper(string? sessionID)
		{
            SessionID = sessionID;
            Items = MockData();
            Page = new(Convert.ToUInt32(5), Convert.ToUInt32(Items?.Count));
		}
        private List<Personnel>? MockData()
			{
            if (Items == null || Items.Count == 0)
                Items = [];

            Items.Add(new Personnel("Alpha-Personnel", 1));
            Items.Add(new Personnel("Beta-Personnel", 2));
            Items.Add(new Personnel("Charlie-Personnel", 3));
            Items.Add(new Personnel("Delta-Personnel", 3));
            Items.Add(new Personnel("Echo-Personnel", 3));
            Items.Add(new Personnel("Foxtrot-Personnel", 3));
            Items.Add(new Personnel("Golf-Personnel", 3));
            Items.Add(new Personnel("Hotel-Personnel", 3));
            return Items;
			}
        ...
        public bool View(U.Navigation navigate = U.Navigation.FirstPage)
           ...
           EntitiesOnThePage = U.ViewWithPagination("Personnel", Page, orderedEntities, navigate);
            return true;
			}
        ...
        public bool Edit(int? simChoice, string[]? simInput)
        {
	        ...
	        try
	        {
	            Personnel? Entity = ViewAndSelectItem(simInput?[0], "Select an item to edit");
        ...
        public bool Search(int? simChoice = null, string[]? simInput = null)
            ...
            else 
            {
                ...
                List<Personnel>? Entities = Items?.Where(c => c.Name != null && c.Name.ToUpper().Contains(Input.ToUpper(), StringComparison.CurrentCultureIgnoreCase)).ToList<Personnel>();
                ...
                else 
                {
                    ...
                    EntitiesOnThePage = [new Personnel("None", 1)];
                    Page = new Patina.Patina(1, 1);
                    return false;
                }
            }
        ...
        public bool Refresh(List<Personnel> modelList, U.Navigation navigate = U.Navigation.FirstPage)
        {
            if (SessionID != null)
                L.Log(System.Reflection.MethodBase.GetCurrentMethod()?.Name, SessionID);
        ...
        public bool Remove(int? simChoice = null, string[]? simInput = null)
            ...
            //List the items
            var CurrentPersonnel = ViewAndSelectItem(simInput?[0], "Select the item to remove");

            if (Items != null && CurrentPersonnel != null)
                if (Items.Remove(CurrentPersonnel))
                {
        ...
        public bool Save(int? simChoice = null, string[]? simInput = null)
            ...
            if (Items != null && Items.Count > 0)
            {
                ...
                try
                {
                    sb.AppendLine($"PersonnelName|PersonnelGuid");
                    foreach (var item in Items.OrderBy(c => c.Name))
                    {
                        sb.AppendLine($"{ item.Name}|{ item.PersonnelGuid}");
                    }
	                ...
	                foreach (string line in FileInput.Split("\r\n"))
	                {
	                    if (!SkipFirstLine)
	                    {
							...
							if (LineItem.Length > 0 && LineItem.Length > 1)
	                        {
	                            ...
	                            var Entity = Items?.FirstOrDefault(c => c.PersonnelGuid == GUID);
	                            var EntityItem = Items?.FirstOrDefault(c => c.Name == Name);
								var OldGUID = CH.IsSimulate ? "<GUID>" : EntityItem?.PersonnelGuid;
								...
								if (Entity == null)
								{
									...
									Items?.Add(new Personnel(Name, 0, GUID));
        ...
        private Personnel? ViewAndSelectItem(string? simInput, string heading)
        {
        ...
        {
            return Items?.FirstOrDefault(p => p.PersonnelGuid != null && p.PersonnelGuid.Equals(guid))?.Name;
        }

        internal string? GetGuid(string? name)
        {
            return Items?.FirstOrDefault(p => p.Name != null && p.Name.Equals(name))?.PersonnelGuid;
        }
    }
}
