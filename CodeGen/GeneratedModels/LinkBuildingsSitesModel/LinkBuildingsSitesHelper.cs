...
{
    public class LinkBuildingsSitesHelper : IHelper<LinkBuildingsSites>, IDisposable
	{
        public string? SessionID { get; set; }
        public List<LinkBuildingsSites>? Items { get; set; }
        public List<LinkBuildingsSites>? EntitiesOnThePage { get; set; }
        public Patina.Patina Page { get; set; }
        public LinkBuildingsSitesMockRepository<LinkBuildingsSites> Repository { get; set; }
       public LinkBuildingsSitesHelper(string? sessionID)
		{
            SessionID = sessionID;
            Items = [];
            Page = new(Convert.ToUInt32(5), Convert.ToUInt32(Items?.Count));
            Repository = new("-LinkBuildingsSites");
		}
        public bool View(U.Navigation navigate = U.Navigation.FirstPage)
           ...
           EntitiesOnThePage = U.ViewWithPagination("LinkBuildingsSites", Page, orderedEntities, navigate);
            return true;
			}
        ...
        public bool Edit(int? simChoice, string[]? simInput)
        {
	        ...
	        try
	        {
	            LinkBuildingsSites? Entity = ViewAndSelectItem(simInput?[0], "Select an item to edit");
	            ...
	            if (Entity != null)
	            {
	                ...
	                if (!string.IsNullOrWhiteSpace(Input))
	                {
	                    ...
	                    if (NotFound == null)
	                    {
	                        EntitiesOnThePage = [new(Input, 0, Entity?.BuildingsGuid, Entity?.SitesGuid)];
        ...
        public bool Search(int? simChoice = null, string[]? simInput = null)
            ...
            else 
            {
                ...
                List<LinkBuildingsSites>? Entities = Items?.Where(c => c.Name != null && c.Name.ToUpper().Contains(Input.ToUpper(), StringComparison.CurrentCultureIgnoreCase)).ToList<LinkBuildingsSites>();
                ...
                else 
                {
                    ...
                    EntitiesOnThePage = [new LinkBuildingsSites("None", 1, null, null)];
                    Page = new Patina.Patina(1, 1);
                    return false;
                }
            }
        ...
        public bool Refresh(List<LinkBuildingsSites> modelList, U.Navigation navigate = U.Navigation.FirstPage)
        {
            if (SessionID != null) { L.Log(System.Reflection.MethodBase.GetCurrentMethod()?.Name, SessionID);}
            ReIndexDisplayId();
            ResetPageMaxCount();
        ...
        public bool Remove(int? simChoice = null, string[]? simInput = null)
            ...
            //List the items
            var CurrentLinkBuildingsSites = ViewAndSelectItem(simInput?[0], "Select the item to remove");

            if (Items != null && CurrentLinkBuildingsSites != null)
                if (Items.Remove(CurrentLinkBuildingsSites))
                {
        ...
        public bool Save(int? simChoice = null, string[]? simInput = null)
            ...
            if (Items != null && Items.Count > 0)
            {
                ...
                try
                {
                    sb.AppendLine($"LinkBuildingsSitesName|LHLink|RHLink|LHLinkName|RHLinkName|Guid|LHLinkGuid|RHLinkGuid");
                    foreach (var item in Items.OrderBy(c => c.Name))
                    {
                        sb.AppendLine($"{ item.Name}|Buildings|Sites|{AL.BuildingsHelper?.GetName(item.BuildingsGuid)}|{AL.SitesHelper?.GetName(item.SitesGuid)}|{ item.LinkBuildingsSitesGuid}|{item.BuildingsGuid}|{item.SitesGuid}");
                    }
	                ...
	                foreach (string line in FileInput.Split("\r\n"))
	                {
	                    if (!SkipFirstLine)
	                    {
							...
							if (LineItem.Length > 0 && LineItem.Length > 1)
	                        {
	                            ...
	                            var Entity = Items?.FirstOrDefault(c => c.LinkBuildingsSitesGuid == GUID);
	                            var EntityItem = Items?.FirstOrDefault(c => c.Name == Name);
								var OldGUID = CH.IsSimulate ? "<GUID>" : EntityItem?.LinkBuildingsSitesGuid;
								...
								if (Entity == null)
								{
									...
									Items?.Add(new LinkBuildingsSites(Name, 0, LHLinkGuid, RHLinkGuid, GUID));
        ...
        public void ReIndexDisplayId()
        {
            ...
            if (OrderedModels != null)
                ...
                {
                    LinkBuildingsSites? item = OrderedModels[i];
                    item.DisplayId = i + 1;
        ...
        private LinkBuildingsSites? ViewAndSelectItem(string? simInput, string heading)
        {
        ...
        {
            return Items?.FirstOrDefault(p => p.LinkBuildingsSitesGuid != null && p.LinkBuildingsSitesGuid.Equals(guid))?.Name;
        }
        internal string? GetGuid(string? name)
        {
            return Items?.FirstOrDefault(p => p.Name != null && p.Name.Equals(name))?.LinkBuildingsSitesGuid;
        }
	}
}
