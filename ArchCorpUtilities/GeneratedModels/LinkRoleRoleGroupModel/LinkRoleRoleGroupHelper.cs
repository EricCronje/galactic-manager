...
{
    public class LinkRoleRoleGroupHelper : IHelper<LinkRoleRoleGroup>, IDisposable
	{
        public string? SessionID { get; set; }
        public List<LinkRoleRoleGroup>? Items { get; set; }
        public List<LinkRoleRoleGroup>? EntitiesOnThePage { get; set; }
        public Patina.Patina Page { get; set; }
        public LinkRoleRoleGroupMockRepository<LinkRoleRoleGroup> Repository { get; set; }
       public LinkRoleRoleGroupHelper(string? sessionID)
		{
            SessionID = sessionID;
            Items = [];
            Page = new(Convert.ToUInt32(5), Convert.ToUInt32(Items?.Count));
            Repository = new("-LinkRoleRoleGroup");
		}
        public bool View(U.Navigation navigate = U.Navigation.FirstPage)
           ...
           EntitiesOnThePage = U.ViewWithPagination("LinkRoleRoleGroup", Page, orderedEntities, navigate);
            return true;
			}
        ...
        public bool Edit(int? simChoice, string[]? simInput)
        {
	        ...
	        try
	        {
	            LinkRoleRoleGroup? Entity = ViewAndSelectItem(simInput?[0], "Select an item to edit");
	            ...
	            if (Entity != null)
	            {
	                ...
	                if (!string.IsNullOrWhiteSpace(Input))
	                {
	                    ...
	                    if (NotFound == null)
	                    {
	                        EntitiesOnThePage = [new(Input, 0, Entity?.RolesGuid, Entity?.RoleGroupsGuid)];
        ...
        public bool Search(int? simChoice = null, string[]? simInput = null)
            ...
            else 
            {
                ...
                List<LinkRoleRoleGroup>? Entities = Items?.Where(c => c.Name != null && c.Name.ToUpper().Contains(Input.ToUpper(), StringComparison.CurrentCultureIgnoreCase)).ToList<LinkRoleRoleGroup>();
                ...
                else 
                {
                    ...
                    EntitiesOnThePage = [new LinkRoleRoleGroup("None", 1, null, null)];
                    Page = new Patina.Patina(1, 1);
                    return false;
                }
            }
        ...
        public bool Refresh(List<LinkRoleRoleGroup> modelList, U.Navigation navigate = U.Navigation.FirstPage)
        {
            if (SessionID != null) { L.Log(System.Reflection.MethodBase.GetCurrentMethod()?.Name, SessionID);}
            ReIndexDisplayId();
            ResetPageMaxCount();
        ...
        public bool Remove(int? simChoice = null, string[]? simInput = null)
            ...
            //List the items
            var CurrentLinkRoleRoleGroup = ViewAndSelectItem(simInput?[0], "Select the item to remove");

            if (Items != null && CurrentLinkRoleRoleGroup != null)
                if (Items.Remove(CurrentLinkRoleRoleGroup))
                {
        ...
        public bool Save(int? simChoice = null, string[]? simInput = null)
            ...
            if (Items != null && Items.Count > 0)
            {
                ...
                try
                {
                    sb.AppendLine($"LinkRoleRoleGroupName|LHLink|RHLink|LHLinkName|RHLinkName|Guid|LHLinkGuid|RHLinkGuid");
                    foreach (var item in Items.OrderBy(c => c.Name))
                    {
                        sb.AppendLine($"{ item.Name}|Roles|RoleGroups|{AL.RolesHelper?.GetName(item.RolesGuid)}|{AL.RoleGroupsHelper?.GetName(item.RoleGroupsGuid)}|{ item.LinkRoleRoleGroupGuid}|{item.RolesGuid}|{item.RoleGroupsGuid}");
                    }
	                ...
	                foreach (string line in FileInput.Split("\r\n"))
	                {
	                    if (!SkipFirstLine)
	                    {
							...
							if (LineItem.Length > 0 && LineItem.Length > 1)
	                        {
	                            ...
	                            var Entity = Items?.FirstOrDefault(c => c.LinkRoleRoleGroupGuid == GUID);
	                            var EntityItem = Items?.FirstOrDefault(c => c.Name == Name);
								var OldGUID = CH.IsSimulate ? "<GUID>" : EntityItem?.LinkRoleRoleGroupGuid;
								...
								if (Entity == null)
								{
									...
									Items?.Add(new LinkRoleRoleGroup(Name, 0, LHLinkGuid, RHLinkGuid, GUID));
        ...
        public void ReIndexDisplayId()
        {
            ...
            if (OrderedModels != null)
                ...
                {
                    LinkRoleRoleGroup? item = OrderedModels[i];
                    item.DisplayId = i + 1;
        ...
        private LinkRoleRoleGroup? ViewAndSelectItem(string? simInput, string heading)
        {
        ...
        {
            return Items?.FirstOrDefault(p => p.LinkRoleRoleGroupGuid != null && p.LinkRoleRoleGroupGuid.Equals(guid))?.Name;
        }
        internal string? GetGuid(string? name)
        {
            return Items?.FirstOrDefault(p => p.Name != null && p.Name.Equals(name))?.LinkRoleRoleGroupGuid;
        }
	}
}
