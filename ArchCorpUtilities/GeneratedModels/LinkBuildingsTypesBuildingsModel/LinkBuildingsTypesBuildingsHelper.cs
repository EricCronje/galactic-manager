...
{
    public class LinkBuildingsTypesBuildingsHelper : IHelper<LinkBuildingsTypesBuildings>, IDisposable
	{
        public string? SessionID { get; set; }
        public List<LinkBuildingsTypesBuildings>? Items { get; set; }
        public List<LinkBuildingsTypesBuildings>? EntitiesOnThePage { get; set; }
        public Patina.Patina Page { get; set; }
        public LinkBuildingsTypesBuildingsMockRepository<LinkBuildingsTypesBuildings> Repository { get; set; }
       public LinkBuildingsTypesBuildingsHelper(string? sessionID)
		{
            SessionID = sessionID;
            Items = [];
            Page = new(Convert.ToUInt32(5), Convert.ToUInt32(Items?.Count));
            Repository = new("-LinkBuildingsTypesBuildings");
		}
        public bool View(U.Navigation navigate = U.Navigation.FirstPage)
           ...
           EntitiesOnThePage = U.ViewWithPagination("LinkBuildingsTypesBuildings", Page, orderedEntities, navigate);
            return true;
			}
        ...
        public bool Edit(int? simChoice, string[]? simInput)
        {
	        ...
	        try
	        {
	            LinkBuildingsTypesBuildings? Entity = ViewAndSelectItem(simInput?[0], "Select an item to edit");
	            ...
	            if (Entity != null)
	            {
	                ...
	                if (!string.IsNullOrWhiteSpace(Input))
	                {
	                    ...
	                    if (NotFound == null)
	                    {
	                        EntitiesOnThePage = [new(Input, 0, Entity?.BuildingTypeGuid, Entity?.BuildingsGuid)];
        ...
        public bool Search(int? simChoice = null, string[]? simInput = null)
            ...
            else 
            {
                ...
                List<LinkBuildingsTypesBuildings>? Entities = Items?.Where(c => c.Name != null && c.Name.ToUpper().Contains(Input.ToUpper(), StringComparison.CurrentCultureIgnoreCase)).ToList<LinkBuildingsTypesBuildings>();
                ...
                else 
                {
                    ...
                    EntitiesOnThePage = [new LinkBuildingsTypesBuildings("None", 1, null, null)];
                    Page = new Patina.Patina(1, 1);
                    return false;
                }
            }
        ...
        public bool Refresh(List<LinkBuildingsTypesBuildings> modelList, U.Navigation navigate = U.Navigation.FirstPage)
        {
            if (SessionID != null) { L.Log(System.Reflection.MethodBase.GetCurrentMethod()?.Name, SessionID);}
            ReIndexDisplayId();
            ResetPageMaxCount();
        ...
        public bool Remove(int? simChoice = null, string[]? simInput = null)
            ...
            //List the items
            var CurrentLinkBuildingsTypesBuildings = ViewAndSelectItem(simInput?[0], "Select the item to remove");

            if (Items != null && CurrentLinkBuildingsTypesBuildings != null)
                if (Items.Remove(CurrentLinkBuildingsTypesBuildings))
                {
        ...
        public bool Save(int? simChoice = null, string[]? simInput = null)
            ...
            var Path = "LinkBuildingsTypesBuildings";
            ...
            if (Items != null && Items.Count > 0)
            {
                ...
                try
                {
                    sb.AppendLine($"LinkBuildingsTypesBuildingsName|LHLink|RHLink|LHLinkName|RHLinkName|Guid|LHLinkGuid|RHLinkGuid");
                    foreach (var item in Items.OrderBy(c => c.Name))
                    {
                        sb.AppendLine($"{ item.Name}|BuildingType|Buildings|{AL.BuildingTypeHelper?.GetName(item.BuildingTypeGuid)}|{AL.BuildingsHelper?.GetName(item.BuildingsGuid)}|{ item.LinkBuildingsTypesBuildingsGuid}|{item.BuildingTypeGuid}|{item.BuildingsGuid}");
                    }
	                ...
	                foreach (string line in FileInput.Split("\r\n"))
	                {
	                    if (!SkipFirstLine)
	                    {
							...
							if (LineItem.Length > 0 && LineItem.Length > 1)
	                        {
	                            ...
	                            var Entity = Items?.FirstOrDefault(c => c.LinkBuildingsTypesBuildingsGuid == GUID);
	                            var EntityItem = Items?.FirstOrDefault(c => c.Name == Name);
								var OldGUID = CH.IsSimulate ? "<GUID>" : EntityItem?.LinkBuildingsTypesBuildingsGuid;
								...
								if (Entity == null)
								{
									...
									Items?.Add(new LinkBuildingsTypesBuildings(Name, 0, LHLinkGuid, RHLinkGuid, GUID));
        ...
        public void ReIndexDisplayId()
        {
            ...
            if (OrderedModels != null)
                ...
                {
                    LinkBuildingsTypesBuildings? item = OrderedModels[i];
                    item.DisplayId = i + 1;
        ...
        private LinkBuildingsTypesBuildings? ViewAndSelectItem(string? simInput, string heading)
        {
        ...
        {
            return Items?.FirstOrDefault(p => p.LinkBuildingsTypesBuildingsGuid != null && p.LinkBuildingsTypesBuildingsGuid.Equals(guid))?.Name;
        }
        internal string? GetGuid(string? name)
        {
            return Items?.FirstOrDefault(p => p.Name != null && p.Name.Equals(name))?.LinkBuildingsTypesBuildingsGuid;
        }
	}
}
