...
{
    public class LinkShipFleetHelper : IHelper<LinkShipFleet>, IDisposable
	{
        public string? SessionID { get; set; }
        public List<LinkShipFleet>? Items { get; set; }
        public List<LinkShipFleet>? EntitiesOnThePage { get; set; }
        public Patina.Patina Page { get; set; }
        public LinkShipFleetMockRepository<LinkShipFleet> Repository { get; set; }
       public LinkShipFleetHelper(string? sessionID)
		{
            SessionID = sessionID;
            Items = [];
            Page = new(Convert.ToUInt32(5), Convert.ToUInt32(Items?.Count));
            Repository = new("-LinkShipFleet");
		}
        public bool View(U.Navigation navigate = U.Navigation.FirstPage)
           ...
           EntitiesOnThePage = U.ViewWithPagination("LinkShipFleet", Page, orderedEntities, navigate);
            return true;
			}
        ...
        public bool Edit(int? simChoice, string[]? simInput)
        {
	        ...
	        try
	        {
	            LinkShipFleet? Entity = ViewAndSelectItem(simInput?[0], "Select an item to edit");
	            ...
	            if (Entity != null)
	            {
	                ...
	                if (!string.IsNullOrWhiteSpace(Input))
	                {
	                    ...
	                    if (NotFound == null)
	                    {
	                        EntitiesOnThePage = [new(Input, 0, Entity?.ShipsGuid, Entity?.FleetsGuid)];
        ...
        public bool Search(int? simChoice = null, string[]? simInput = null)
            ...
            else 
            {
                ...
                List<LinkShipFleet>? Entities = Items?.Where(c => c.Name != null && c.Name.ToUpper().Contains(Input.ToUpper(), StringComparison.CurrentCultureIgnoreCase)).ToList<LinkShipFleet>();
                ...
                else 
                {
                    ...
                    EntitiesOnThePage = [new LinkShipFleet("None", 1, null, null)];
                    Page = new Patina.Patina(1, 1);
                    return false;
                }
            }
        ...
        public bool Refresh(List<LinkShipFleet> modelList, U.Navigation navigate = U.Navigation.FirstPage)
        {
            if (SessionID != null) { L.Log(System.Reflection.MethodBase.GetCurrentMethod()?.Name, SessionID);}
            ReIndexDisplayId();
            ResetPageMaxCount();
        ...
        public bool Remove(int? simChoice = null, string[]? simInput = null)
            ...
            //List the items
            var CurrentLinkShipFleet = ViewAndSelectItem(simInput?[0], "Select the item to remove");

            if (Items != null && CurrentLinkShipFleet != null)
                if (Items.Remove(CurrentLinkShipFleet))
                {
        ...
        public bool Save(int? simChoice = null, string[]? simInput = null)
            ...
            if (Items != null && Items.Count > 0)
            {
                ...
                try
                {
                    sb.AppendLine($"LinkShipFleetName|LHLink|RHLink|LHLinkName|RHLinkName|Guid|LHLinkGuid|RHLinkGuid");
                    foreach (var item in Items.OrderBy(c => c.Name))
                    {
                        sb.AppendLine($"{ item.Name}|Ships|Fleets|{AL.ShipsHelper?.GetName(item.ShipsGuid)}|{AL.FleetsHelper?.GetName(item.FleetsGuid)}|{ item.LinkShipFleetGuid}|{item.ShipsGuid}|{item.FleetsGuid}");
                    }
	                ...
	                foreach (string line in FileInput.Split("\r\n"))
	                {
	                    if (!SkipFirstLine)
	                    {
							...
							if (LineItem.Length > 0 && LineItem.Length > 1)
	                        {
	                            ...
	                            var Entity = Items?.FirstOrDefault(c => c.LinkShipFleetGuid == GUID);
	                            var EntityItem = Items?.FirstOrDefault(c => c.Name == Name);
								var OldGUID = CH.IsSimulate ? "<GUID>" : EntityItem?.LinkShipFleetGuid;
								...
								if (Entity == null)
								{
									...
									Items?.Add(new LinkShipFleet(Name, 0, LHLinkGuid, RHLinkGuid, GUID));
        ...
        public void ReIndexDisplayId()
        {
            ...
            if (OrderedModels != null)
                ...
                {
                    LinkShipFleet? item = OrderedModels[i];
                    item.DisplayId = i + 1;
        ...
        private LinkShipFleet? ViewAndSelectItem(string? simInput, string heading)
        {
        ...
        {
            return Items?.FirstOrDefault(p => p.LinkShipFleetGuid != null && p.LinkShipFleetGuid.Equals(guid))?.Name;
        }
        internal string? GetGuid(string? name)
        {
            return Items?.FirstOrDefault(p => p.Name != null && p.Name.Equals(name))?.LinkShipFleetGuid;
        }
	}
}
