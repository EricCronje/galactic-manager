...
{
    public class LinkBeaconRouteHelper : IHelper<LinkBeaconRoute>, IDisposable
	{
        public string? SessionID { get; set; }
        public List<LinkBeaconRoute>? Items { get; set; }
        public List<LinkBeaconRoute>? EntitiesOnThePage { get; set; }
        public Patina.Patina Page { get; set; }
        public LinkBeaconRouteMockRepository<LinkBeaconRoute> Repository { get; set; }
       public LinkBeaconRouteHelper(string? sessionID)
		{
            SessionID = sessionID;
            Items = [];
            Page = new(Convert.ToUInt32(5), Convert.ToUInt32(Items?.Count));
            Repository = new("-LinkBeaconRoute");
		}
        public bool View(U.Navigation navigate = U.Navigation.FirstPage)
           ...
           EntitiesOnThePage = U.ViewWithPagination("LinkBeaconRoute", Page, orderedEntities, navigate);
            return true;
			}
        ...
        public bool Edit(int? simChoice, string[]? simInput)
        {
	        ...
	        try
	        {
	            LinkBeaconRoute? Entity = ViewAndSelectItem(simInput?[0], "Select an item to edit");
	            ...
	            if (Entity != null)
	            {
	                ...
	                if (!string.IsNullOrWhiteSpace(Input))
	                {
	                    ...
	                    if (NotFound == null)
	                    {
	                        EntitiesOnThePage = [new(Input, 0, Entity?.BeaconsGuid, Entity?.RoutesGuid)];
        ...
        public bool Search(int? simChoice = null, string[]? simInput = null)
            ...
            else 
            {
                ...
                List<LinkBeaconRoute>? Entities = Items?.Where(c => c.Name != null && c.Name.ToUpper().Contains(Input.ToUpper(), StringComparison.CurrentCultureIgnoreCase)).ToList<LinkBeaconRoute>();
                ...
                else 
                {
                    ...
                    EntitiesOnThePage = [new LinkBeaconRoute("None", 1, null, null)];
                    Page = new Patina.Patina(1, 1);
                    return false;
                }
            }
        ...
        public bool Refresh(List<LinkBeaconRoute> modelList, U.Navigation navigate = U.Navigation.FirstPage)
        {
            if (SessionID != null) { L.Log(System.Reflection.MethodBase.GetCurrentMethod()?.Name, SessionID);}
            ReIndexDisplayId();
            ResetPageMaxCount();
        ...
        public bool Remove(int? simChoice = null, string[]? simInput = null)
            ...
            //List the items
            var CurrentLinkBeaconRoute = ViewAndSelectItem(simInput?[0], "Select the item to remove");

            if (Items != null && CurrentLinkBeaconRoute != null)
                if (Items.Remove(CurrentLinkBeaconRoute))
                {
        ...
        public bool Save(int? simChoice = null, string[]? simInput = null)
            ...
            if (Items != null && Items.Count > 0)
            {
                ...
                try
                {
                    sb.AppendLine($"LinkBeaconRouteName|LHLink|RHLink|LHLinkName|RHLinkName|Guid|LHLinkGuid|RHLinkGuid");
                    foreach (var item in Items.OrderBy(c => c.Name))
                    {
                        sb.AppendLine($"{ item.Name}|Beacons|Routes|{AL.BeaconsHelper?.GetName(item.BeaconsGuid)}|{AL.RoutesHelper?.GetName(item.RoutesGuid)}|{ item.LinkBeaconRouteGuid}|{item.BeaconsGuid}|{item.RoutesGuid}");
                    }
	                ...
	                foreach (string line in FileInput.Split("\r\n"))
	                {
	                    if (!SkipFirstLine)
	                    {
							...
							if (LineItem.Length > 0 && LineItem.Length > 1)
	                        {
	                            ...
	                            var Entity = Items?.FirstOrDefault(c => c.LinkBeaconRouteGuid == GUID);
	                            var EntityItem = Items?.FirstOrDefault(c => c.Name == Name);
								var OldGUID = CH.IsSimulate ? "<GUID>" : EntityItem?.LinkBeaconRouteGuid;
								...
								if (Entity == null)
								{
									...
									Items?.Add(new LinkBeaconRoute(Name, 0, LHLinkGuid, RHLinkGuid, GUID));
        ...
        public void ReIndexDisplayId()
        {
            ...
            if (OrderedModels != null)
                ...
                {
                    LinkBeaconRoute? item = OrderedModels[i];
                    item.DisplayId = i + 1;
        ...
        private LinkBeaconRoute? ViewAndSelectItem(string? simInput, string heading)
        {
        ...
        {
            return Items?.FirstOrDefault(p => p.LinkBeaconRouteGuid != null && p.LinkBeaconRouteGuid.Equals(guid))?.Name;
        }
        internal string? GetGuid(string? name)
        {
            return Items?.FirstOrDefault(p => p.Name != null && p.Name.Equals(name))?.LinkBeaconRouteGuid;
        }
	}
}
