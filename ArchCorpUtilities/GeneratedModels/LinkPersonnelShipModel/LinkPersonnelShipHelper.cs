...
{
    public class LinkPersonnelShipHelper : IHelper<LinkPersonnelShip>, IDisposable
	{
        public string? SessionID { get; set; }
        public List<LinkPersonnelShip>? Items { get; set; }
        public List<LinkPersonnelShip>? EntitiesOnThePage { get; set; }
        public Patina.Patina Page { get; set; }
        public LinkPersonnelShipMockRepository<LinkPersonnelShip> Repository { get; set; }
       public LinkPersonnelShipHelper(string? sessionID)
		{
            SessionID = sessionID;
            Items = [];
            Page = new(Convert.ToUInt32(5), Convert.ToUInt32(Items?.Count));
            Repository = new("-LinkPersonnelShip");
		}
        public bool View(U.Navigation navigate = U.Navigation.FirstPage)
           ...
           EntitiesOnThePage = U.ViewWithPagination("LinkPersonnelShip", Page, orderedEntities, navigate);
            return true;
			}
        ...
        public bool Edit(int? simChoice, string[]? simInput)
        {
	        ...
	        try
	        {
	            LinkPersonnelShip? Entity = ViewAndSelectItem(simInput?[0], "Select an item to edit");
	            ...
	            if (Entity != null)
	            {
	                ...
	                if (!string.IsNullOrWhiteSpace(Input))
	                {
	                    ...
	                    if (NotFound == null)
	                    {
	                        EntitiesOnThePage = [new(Input, 0, Entity?.PersonnelGuid, Entity?.ShipsGuid)];
        ...
        public bool Search(int? simChoice = null, string[]? simInput = null)
            ...
            else 
            {
                ...
                List<LinkPersonnelShip>? Entities = Items?.Where(c => c.Name != null && c.Name.ToUpper().Contains(Input.ToUpper(), StringComparison.CurrentCultureIgnoreCase)).ToList<LinkPersonnelShip>();
                ...
                else 
                {
                    ...
                    EntitiesOnThePage = [new LinkPersonnelShip("None", 1, null, null)];
                    Page = new Patina.Patina(1, 1);
                    return false;
                }
            }
        ...
        public bool Refresh(List<LinkPersonnelShip> modelList, U.Navigation navigate = U.Navigation.FirstPage)
        {
            if (SessionID != null) { L.Log(System.Reflection.MethodBase.GetCurrentMethod()?.Name, SessionID);}
            ReIndexDisplayId();
            ResetPageMaxCount();
        ...
        public bool Remove(int? simChoice = null, string[]? simInput = null)
            ...
            //List the items
            var CurrentLinkPersonnelShip = ViewAndSelectItem(simInput?[0], "Select the item to remove");

            if (Items != null && CurrentLinkPersonnelShip != null)
                if (Items.Remove(CurrentLinkPersonnelShip))
                {
        ...
        public bool Save(int? simChoice = null, string[]? simInput = null)
            ...
            if (Items != null && Items.Count > 0)
            {
                ...
                try
                {
                    sb.AppendLine($"LinkPersonnelShipName|LHLink|RHLink|LHLinkName|RHLinkName|Guid|LHLinkGuid|RHLinkGuid");
                    foreach (var item in Items.OrderBy(c => c.Name))
                    {
                        sb.AppendLine($"{ item.Name}|Personnel|Ships|{AL.PersonnelHelper?.GetName(item.PersonnelGuid)}|{AL.ShipsHelper?.GetName(item.ShipsGuid)}|{ item.LinkPersonnelShipGuid}|{item.PersonnelGuid}|{item.ShipsGuid}");
                    }
	                ...
	                foreach (string line in FileInput.Split("\r\n"))
	                {
	                    if (!SkipFirstLine)
	                    {
							...
							if (LineItem.Length > 0 && LineItem.Length > 1)
	                        {
	                            ...
	                            var Entity = Items?.FirstOrDefault(c => c.LinkPersonnelShipGuid == GUID);
	                            var EntityItem = Items?.FirstOrDefault(c => c.Name == Name);
								var OldGUID = CH.IsSimulate ? "<GUID>" : EntityItem?.LinkPersonnelShipGuid;
								...
								if (Entity == null)
								{
									...
									Items?.Add(new LinkPersonnelShip(Name, 0, LHLinkGuid, RHLinkGuid, GUID));
        ...
        public void ReIndexDisplayId()
        {
            ...
            if (OrderedModels != null)
                ...
                {
                    LinkPersonnelShip? item = OrderedModels[i];
                    item.DisplayId = i + 1;
        ...
        private LinkPersonnelShip? ViewAndSelectItem(string? simInput, string heading)
        {
        ...
        {
            return Items?.FirstOrDefault(p => p.LinkPersonnelShipGuid != null && p.LinkPersonnelShipGuid.Equals(guid))?.Name;
        }
        internal string? GetGuid(string? name)
        {
            return Items?.FirstOrDefault(p => p.Name != null && p.Name.Equals(name))?.LinkPersonnelShipGuid;
        }
	}
}
