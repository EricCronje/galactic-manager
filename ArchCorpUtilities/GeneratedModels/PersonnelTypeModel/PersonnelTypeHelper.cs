...
{
    public class PersonnelTypeHelper : IHelper<PersonnelType>, IDisposable
	{
        public string? SessionID { get; set; }
        public List<PersonnelType>? Items { get; set; }
        public List<PersonnelType>? EntitiesOnThePage { get; set; }
        public Patina.Patina Page { get; set; }
        public PersonnelTypeMockRepository<PersonnelType> Repository { get; set; }
       public PersonnelTypeHelper(string? sessionID)
		{
            SessionID = sessionID;
            Items = [];
            Page = new(Convert.ToUInt32(5), Convert.ToUInt32(Items?.Count));
            Repository = new("-PersonnelType");
		}
        public bool View(U.Navigation navigate = U.Navigation.FirstPage)
           ...
           EntitiesOnThePage = U.ViewWithPagination("PersonnelType", Page, orderedEntities, navigate);
            return true;
			}
        ...
        public bool Edit(int? simChoice, string[]? simInput)
        {
	        ...
	        try
	        {
	            PersonnelType? Entity = ViewAndSelectItem(simInput?[0], "Select an item to edit");
        ...
        public bool Search(int? simChoice = null, string[]? simInput = null)
            ...
            else 
            {
                ...
                List<PersonnelType>? Entities = Items?.Where(c => c.Name != null && c.Name.ToUpper().Contains(Input.ToUpper(), StringComparison.CurrentCultureIgnoreCase)).ToList<PersonnelType>();
                ...
                else 
                {
                    ...
                    EntitiesOnThePage = [new PersonnelType("None", 1)];
                    Page = new Patina.Patina(1, 1);
                    return false;
                }
            }
        ...
        public bool Refresh(List<PersonnelType> modelList, U.Navigation navigate = U.Navigation.FirstPage)
        {
            if (SessionID != null) { L.Log(System.Reflection.MethodBase.GetCurrentMethod()?.Name, SessionID);}
            ReIndexDisplayId();
            ResetPageMaxCount();
        ...
        public bool Remove(int? simChoice = null, string[]? simInput = null)
            ...
            //List the items
            var CurrentPersonnelType = ViewAndSelectItem(simInput?[0], "Select the item to remove");

            if (Items != null && CurrentPersonnelType != null)
                if (Items.Remove(CurrentPersonnelType))
                {
        ...
        public bool Save(int? simChoice = null, string[]? simInput = null)
            ...
            if (Items != null && Items.Count > 0)
            {
                ...
                try
                {
                    sb.AppendLine($"PersonnelTypeName|PersonnelTypeGuid");
                    foreach (var item in Items.OrderBy(c => c.Name))
                    {
                        sb.AppendLine($"{ item.Name}|{ item.PersonnelTypeGuid}");
                    }
	                ...
	                foreach (string line in FileInput.Split("\r\n"))
	                {
	                    if (!SkipFirstLine)
	                    {
							...
							if (LineItem.Length > 0 && LineItem.Length > 1)
	                        {
	                            ...
	                            var Entity = Items?.FirstOrDefault(c => c.PersonnelTypeGuid == GUID);
	                            var EntityItem = Items?.FirstOrDefault(c => c.Name == Name);
								var OldGUID = CH.IsSimulate ? "<GUID>" : EntityItem?.PersonnelTypeGuid;
								...
								if (Entity == null)
								{
									...
									Items?.Add(new PersonnelType(Name, 0, GUID));
        ...
        public void ReIndexDisplayId()
        {
            ...
            if (OrderedModels != null)
                ...
                {
                    PersonnelType? item = OrderedModels[i];
                    item.DisplayId = i + 1;
        ...
        private PersonnelType? ViewAndSelectItem(string? simInput, string heading)
        {
        ...
        {
            return Items?.FirstOrDefault(p => p.PersonnelTypeGuid != null && p.PersonnelTypeGuid.Equals(guid))?.Name;
        }
        internal string? GetGuid(string? name)
        {
            return Items?.FirstOrDefault(p => p.Name != null && p.Name.Equals(name))?.PersonnelTypeGuid;
        }
	}
}
