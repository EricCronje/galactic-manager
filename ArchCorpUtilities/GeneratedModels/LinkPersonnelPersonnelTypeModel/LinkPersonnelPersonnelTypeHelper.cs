...
{
    public class LinkPersonnelPersonnelTypeHelper : IHelper<LinkPersonnelPersonnelType>, IDisposable
	{
        public string? SessionID { get; set; }
        public List<LinkPersonnelPersonnelType>? Items { get; set; }
        public List<LinkPersonnelPersonnelType>? EntitiesOnThePage { get; set; }
        public Patina.Patina Page { get; set; }
        public LinkPersonnelPersonnelTypeMockRepository<LinkPersonnelPersonnelType> Repository { get; set; }
       public LinkPersonnelPersonnelTypeHelper(string? sessionID)
		{
            SessionID = sessionID;
            Items = [];
            Page = new(Convert.ToUInt32(5), Convert.ToUInt32(Items?.Count));
            Repository = new("-LinkPersonnelPersonnelType");
		}
        public bool View(U.Navigation navigate = U.Navigation.FirstPage)
           ...
           EntitiesOnThePage = U.ViewWithPagination("LinkPersonnelPersonnelType", Page, orderedEntities, navigate);
            return true;
			}
        ...
        public bool Edit(int? simChoice, string[]? simInput)
        {
	        ...
	        try
	        {
	            LinkPersonnelPersonnelType? Entity = ViewAndSelectItem(simInput?[0], "Select an item to edit");
	            ...
	            if (Entity != null)
	            {
	                ...
	                if (!string.IsNullOrWhiteSpace(Input))
	                {
	                    ...
	                    if (NotFound == null)
	                    {
	                        EntitiesOnThePage = [new(Input, 0, Entity?.PersonnelGuid, Entity?.PersonnelTypeGuid)];
        ...
        public bool Search(int? simChoice = null, string[]? simInput = null)
            ...
            else 
            {
                ...
                List<LinkPersonnelPersonnelType>? Entities = Items?.Where(c => c.Name != null && c.Name.ToUpper().Contains(Input.ToUpper(), StringComparison.CurrentCultureIgnoreCase)).ToList<LinkPersonnelPersonnelType>();
                ...
                else 
                {
                    ...
                    EntitiesOnThePage = [new LinkPersonnelPersonnelType("None", 1, null, null)];
                    Page = new Patina.Patina(1, 1);
                    return false;
                }
            }
        ...
        public bool Refresh(List<LinkPersonnelPersonnelType> modelList, U.Navigation navigate = U.Navigation.FirstPage)
        {
            if (SessionID != null) { L.Log(System.Reflection.MethodBase.GetCurrentMethod()?.Name, SessionID);}
            ReIndexDisplayId();
            ResetPageMaxCount();
        ...
        public bool Remove(int? simChoice = null, string[]? simInput = null)
            ...
            //List the items
            var CurrentLinkPersonnelPersonnelType = ViewAndSelectItem(simInput?[0], "Select the item to remove");

            if (Items != null && CurrentLinkPersonnelPersonnelType != null)
                if (Items.Remove(CurrentLinkPersonnelPersonnelType))
                {
        ...
        public bool Save(int? simChoice = null, string[]? simInput = null)
            ...
            var Path = "LinkPersonnelPersonnelType";
            ...
            if (Items != null && Items.Count > 0)
            {
                ...
                try
                {
                    sb.AppendLine($"LinkPersonnelPersonnelTypeName|LHLink|RHLink|LHLinkName|RHLinkName|Guid|LHLinkGuid|RHLinkGuid");
                    foreach (var item in Items.OrderBy(c => c.Name))
                    {
                        sb.AppendLine($"{ item.Name}|Personnel|PersonnelType|{AL.PersonnelHelper?.GetName(item.PersonnelGuid)}|{AL.PersonnelTypeHelper?.GetName(item.PersonnelTypeGuid)}|{ item.LinkPersonnelPersonnelTypeGuid}|{item.PersonnelGuid}|{item.PersonnelTypeGuid}");
                    }
	                ...
	                foreach (string line in FileInput.Split("\r\n"))
	                {
	                    if (!SkipFirstLine)
	                    {
							...
							if (LineItem.Length > 0 && LineItem.Length > 1)
	                        {
	                            ...
	                            var Entity = Items?.FirstOrDefault(c => c.LinkPersonnelPersonnelTypeGuid == GUID);
	                            var EntityItem = Items?.FirstOrDefault(c => c.Name == Name);
								var OldGUID = CH.IsSimulate ? "<GUID>" : EntityItem?.LinkPersonnelPersonnelTypeGuid;
								...
								if (Entity == null)
								{
									...
									Items?.Add(new LinkPersonnelPersonnelType(Name, 0, LHLinkGuid, RHLinkGuid, GUID));
        ...
        public void ReIndexDisplayId()
        {
            ...
            if (OrderedModels != null)
                ...
                {
                    LinkPersonnelPersonnelType? item = OrderedModels[i];
                    item.DisplayId = i + 1;
        ...
        private LinkPersonnelPersonnelType? ViewAndSelectItem(string? simInput, string heading)
        {
        ...
        {
            return Items?.FirstOrDefault(p => p.LinkPersonnelPersonnelTypeGuid != null && p.LinkPersonnelPersonnelTypeGuid.Equals(guid))?.Name;
        }
        internal string? GetGuid(string? name)
        {
            return Items?.FirstOrDefault(p => p.Name != null && p.Name.Equals(name))?.LinkPersonnelPersonnelTypeGuid;
        }
	}
}
