...
{
    public class LinkShipTypeShipHelper : IHelper<LinkShipTypeShip>, IDisposable
	{
        public string? SessionID { get; set; }
        public List<LinkShipTypeShip>? Items { get; set; }
        public List<LinkShipTypeShip>? EntitiesOnThePage { get; set; }
        public Patina.Patina Page { get; set; }
        public LinkShipTypeShipMockRepository<LinkShipTypeShip> Repository { get; set; }
       public LinkShipTypeShipHelper(string? sessionID)
		{
            SessionID = sessionID;
            Items = [];
            Page = new(Convert.ToUInt32(5), Convert.ToUInt32(Items?.Count));
            Repository = new("-LinkShipTypeShip");
		}
        public bool View(U.Navigation navigate = U.Navigation.FirstPage)
           ...
           EntitiesOnThePage = U.ViewWithPagination("LinkShipTypeShip", Page, orderedEntities, navigate);
            return true;
			}
        ...
        public bool Edit(int? simChoice, string[]? simInput)
        {
	        ...
	        try
	        {
	            LinkShipTypeShip? Entity = ViewAndSelectItem(simInput?[0], "Select an item to edit");
	            ...
	            if (Entity != null)
	            {
	                ...
	                if (!string.IsNullOrWhiteSpace(Input))
	                {
	                    ...
	                    if (NotFound == null)
	                    {
	                        EntitiesOnThePage = [new(Input, 0, Entity?.ShipTypeGuid, Entity?.ShipsGuid)];
        ...
        public bool Search(int? simChoice = null, string[]? simInput = null)
            ...
            else 
            {
                ...
                List<LinkShipTypeShip>? Entities = Items?.Where(c => c.Name != null && c.Name.ToUpper().Contains(Input.ToUpper(), StringComparison.CurrentCultureIgnoreCase)).ToList<LinkShipTypeShip>();
                ...
                else 
                {
                    ...
                    EntitiesOnThePage = [new LinkShipTypeShip("None", 1, null, null)];
                    Page = new Patina.Patina(1, 1);
                    return false;
                }
            }
        ...
        public bool Refresh(List<LinkShipTypeShip> modelList, U.Navigation navigate = U.Navigation.FirstPage)
        {
            if (SessionID != null) { L.Log(System.Reflection.MethodBase.GetCurrentMethod()?.Name, SessionID);}
            ReIndexDisplayId();
            ResetPageMaxCount();
        ...
        public bool Remove(int? simChoice = null, string[]? simInput = null)
            ...
            //List the items
            var CurrentLinkShipTypeShip = ViewAndSelectItem(simInput?[0], "Select the item to remove");

            if (Items != null && CurrentLinkShipTypeShip != null)
                if (Items.Remove(CurrentLinkShipTypeShip))
                {
        ...
        public bool Save(int? simChoice = null, string[]? simInput = null)
            ...
            if (Items != null && Items.Count > 0)
            {
                ...
                try
                {
                    sb.AppendLine($"LinkShipTypeShipName|LHLink|RHLink|LHLinkName|RHLinkName|Guid|LHLinkGuid|RHLinkGuid");
                    foreach (var item in Items.OrderBy(c => c.Name))
                    {
                        sb.AppendLine($"{ item.Name}|ShipType|Ships|{AL.ShipTypeHelper?.GetName(item.ShipTypeGuid)}|{AL.ShipsHelper?.GetName(item.ShipsGuid)}|{ item.LinkShipTypeShipGuid}|{item.ShipTypeGuid}|{item.ShipsGuid}");
                    }
	                ...
	                foreach (string line in FileInput.Split("\r\n"))
	                {
	                    if (!SkipFirstLine)
	                    {
							...
							if (LineItem.Length > 0 && LineItem.Length > 1)
	                        {
	                            ...
	                            var Entity = Items?.FirstOrDefault(c => c.LinkShipTypeShipGuid == GUID);
	                            var EntityItem = Items?.FirstOrDefault(c => c.Name == Name);
								var OldGUID = CH.IsSimulate ? "<GUID>" : EntityItem?.LinkShipTypeShipGuid;
								...
								if (Entity == null)
								{
									...
									Items?.Add(new LinkShipTypeShip(Name, 0, LHLinkGuid, RHLinkGuid, GUID));
        ...
        public void ReIndexDisplayId()
        {
            ...
            if (OrderedModels != null)
                ...
                {
                    LinkShipTypeShip? item = OrderedModels[i];
                    item.DisplayId = i + 1;
        ...
        private LinkShipTypeShip? ViewAndSelectItem(string? simInput, string heading)
        {
        ...
        {
            return Items?.FirstOrDefault(p => p.LinkShipTypeShipGuid != null && p.LinkShipTypeShipGuid.Equals(guid))?.Name;
        }
        internal string? GetGuid(string? name)
        {
            return Items?.FirstOrDefault(p => p.Name != null && p.Name.Equals(name))?.LinkShipTypeShipGuid;
        }
	}
}
