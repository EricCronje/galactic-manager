...
{
    public class LinkPlanetTypesPlanetHelper : IHelper<LinkPlanetTypesPlanet>, IDisposable
	{
        public string? SessionID { get; set; }
        public List<LinkPlanetTypesPlanet>? Items { get; set; }
        public List<LinkPlanetTypesPlanet>? EntitiesOnThePage { get; set; }
        public Patina.Patina Page { get; set; }
        public LinkPlanetTypesPlanetMockRepository<LinkPlanetTypesPlanet> Repository { get; set; }
       public LinkPlanetTypesPlanetHelper(string? sessionID)
		{
            SessionID = sessionID;
            Items = [];
            Page = new(Convert.ToUInt32(5), Convert.ToUInt32(Items?.Count));
            Repository = new("-LinkPlanetTypesPlanet");
		}
        public bool View(U.Navigation navigate = U.Navigation.FirstPage)
           ...
           EntitiesOnThePage = U.ViewWithPagination("LinkPlanetTypesPlanet", Page, orderedEntities, navigate);
            return true;
			}
        ...
        public bool Edit(int? simChoice, string[]? simInput)
        {
	        ...
	        try
	        {
	            LinkPlanetTypesPlanet? Entity = ViewAndSelectItem(simInput?[0], "Select an item to edit");
	            ...
	            if (Entity != null)
	            {
	                ...
	                if (!string.IsNullOrWhiteSpace(Input))
	                {
	                    ...
	                    if (NotFound == null)
	                    {
	                        EntitiesOnThePage = [new(Input, 0, Entity?.PlanetTypeGuid, Entity?.PlanetsGuid)];
        ...
        public bool Search(int? simChoice = null, string[]? simInput = null)
            ...
            else 
            {
                ...
                List<LinkPlanetTypesPlanet>? Entities = Items?.Where(c => c.Name != null && c.Name.ToUpper().Contains(Input.ToUpper(), StringComparison.CurrentCultureIgnoreCase)).ToList<LinkPlanetTypesPlanet>();
                ...
                else 
                {
                    ...
                    EntitiesOnThePage = [new LinkPlanetTypesPlanet("None", 1, null, null)];
                    Page = new Patina.Patina(1, 1);
                    return false;
                }
            }
        ...
        public bool Refresh(List<LinkPlanetTypesPlanet> modelList, U.Navigation navigate = U.Navigation.FirstPage)
        {
            if (SessionID != null) { L.Log(System.Reflection.MethodBase.GetCurrentMethod()?.Name, SessionID);}
            ReIndexDisplayId();
            ResetPageMaxCount();
        ...
        public bool Remove(int? simChoice = null, string[]? simInput = null)
            ...
            //List the items
            var CurrentLinkPlanetTypesPlanet = ViewAndSelectItem(simInput?[0], "Select the item to remove");

            if (Items != null && CurrentLinkPlanetTypesPlanet != null)
                if (Items.Remove(CurrentLinkPlanetTypesPlanet))
                {
        ...
        public bool Save(int? simChoice = null, string[]? simInput = null)
            ...
            var Path = "LinkPlanetTypesPlanet";
            ...
            if (Items != null && Items.Count > 0)
            {
                ...
                try
                {
                    sb.AppendLine($"LinkPlanetTypesPlanetName|LHLink|RHLink|LHLinkName|RHLinkName|Guid|LHLinkGuid|RHLinkGuid");
                    foreach (var item in Items.OrderBy(c => c.Name))
                    {
                        sb.AppendLine($"{ item.Name}|PlanetType|Planets|{AL.PlanetTypeHelper?.GetName(item.PlanetTypeGuid)}|{AL.PlanetsHelper?.GetName(item.PlanetsGuid)}|{ item.LinkPlanetTypesPlanetGuid}|{item.PlanetTypeGuid}|{item.PlanetsGuid}");
                    }
	                ...
	                foreach (string line in FileInput.Split("\r\n"))
	                {
	                    if (!SkipFirstLine)
	                    {
							...
							if (LineItem.Length > 0 && LineItem.Length > 1)
	                        {
	                            ...
	                            var Entity = Items?.FirstOrDefault(c => c.LinkPlanetTypesPlanetGuid == GUID);
	                            var EntityItem = Items?.FirstOrDefault(c => c.Name == Name);
								var OldGUID = CH.IsSimulate ? "<GUID>" : EntityItem?.LinkPlanetTypesPlanetGuid;
								...
								if (Entity == null)
								{
									...
									Items?.Add(new LinkPlanetTypesPlanet(Name, 0, LHLinkGuid, RHLinkGuid, GUID));
        ...
        public void ReIndexDisplayId()
        {
            ...
            if (OrderedModels != null)
                ...
                {
                    LinkPlanetTypesPlanet? item = OrderedModels[i];
                    item.DisplayId = i + 1;
        ...
        private LinkPlanetTypesPlanet? ViewAndSelectItem(string? simInput, string heading)
        {
        ...
        {
            return Items?.FirstOrDefault(p => p.LinkPlanetTypesPlanetGuid != null && p.LinkPlanetTypesPlanetGuid.Equals(guid))?.Name;
        }
        internal string? GetGuid(string? name)
        {
            return Items?.FirstOrDefault(p => p.Name != null && p.Name.Equals(name))?.LinkPlanetTypesPlanetGuid;
        }
	}
}
